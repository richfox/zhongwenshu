...
def _insert_product(connection, dbname, book, image_urls, sn=None):
    ...
    sql = """INSERT INTO {goods} (`goods_id`, `cat_id`, `goods_sn`, `goods_name`,
        ...
        `is_delete`, `is_best`, `is_new`, `is_hot`, `is_promote`, `bonus_type_id`,
        `last_update`, `goods_type`, `seller_note`, `give_integral`, `rank_integral`,
        `suppliers_id`, `is_check`) VALUES (NULL, '134', %s, %s, '+', '0', '0', '',
        '0', '0.000', %s, '', %s, '0.00', '0', '0', '1', '', '', %s, %s, %s, %s,
        '1', '', '0', '1', '0', '0', %s, '100', '0', '0', '1', '0', '0', '0',
        '0', '1', '', '-1', '-1', '0', NULL)""".format(goods=goods)
    with connection.cursor() as cursor:
        cursor.execute(sql, (sn, book['title'], market_price, shop_price,
                             book['description'], image_urls['thumb'],
                             image_urls['goods'], image_urls['ori'],
                             str(int(time.time()))))
